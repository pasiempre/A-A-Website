...
// ============================================================
// Fallback responses (rule-based, no external dependency)
// ============================================================
...
// ============================================================
// Anthropic integration (with timeout and graceful fallback)
// ============================================================
...
async function callAnthropic(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `[ai-assistant] Anthropic returned ${response.status}`,
      );
      return null;
    }
    ...
    // Timeout, network failure, or parse error — fall back gracefully
    console.warn(
      "[ai-assistant] Anthropic call failed:",
...
export async function POST(request: Request) {
  ...
  const rateLimitResult = await rateLimit(`ai-assistant:${ip}`, "strict");
  ...
  try {
    ...
    if (!sessionId) {
      ...
      if (sessionError || !createdSession) {
        return NextResponse.json(
          {
            error:
              sessionError?.message ?? "Unable to create chat session.",
          },
          { status: 500 },
        );
      }
    ...
    // --- Generate reply (Anthropic with fallback) ---
