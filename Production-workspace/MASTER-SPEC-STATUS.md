...
| SMS notifications (assignment + new lead alerts) | 🟡 | `src/app/api/assignment-notify/route.ts`, `src/lib/assignment-notifications.ts`, `src/app/api/quote-request/route.ts`, `src/lib/notifications.ts` | Logic implemented including quiet-hours queue; status depends on Twilio/env + ops verification. |
...
| Work order assigned + crew receives SMS | 🟡 | Assignment + SMS dispatch implemented; needs verified Twilio production runbook and delivery monitoring. |
...
| Calendar drag-and-drop | Phase 5 | ⬜ Not started as drag/drop calendar UX. |
