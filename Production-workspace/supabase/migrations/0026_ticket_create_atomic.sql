-- 0026_ticket_create_atomic.sql
-- C-16 hardening: enforce atomic ticket creation path (job + assignment + checklist seed)
...
BEGIN
  ...
  VALUES (
    trim(p_title),
    trim(p_address),
    coalesce(nullif(trim(p_clean_type), ''), 'post_construction'),
